    ...
    if (saved) {
      data = data.map(s => {
        if (s.id === "jezyki-skryptowe") {
          ...
          if (defaultJs && (!s.modules || s.modules.length < 12 || !s.modules.some(m => m.id === "js-pe2-final"))) {
            console.log("Migrated 'jezyki-skryptowe' module database to include PE1 and PE2 modules.");
            return defaultJs;
          }
        }
